...
// 全点対最短経路キャッシュ [from_idx][from_dir][to_idx][to_dir]
...
int main() {
    ...
    auto get_score = [&](const vector<int>& current_state) {
        int dp[4] = {INF, 0, INF, INF}; // 初期: (0,0) で右向き (1) がコスト0
        int curr_pos_idx = 0;
        ...
        for (int idx : current_state) {
            ...
            rep(d, 4) dp[d] = next_dp_b[d] + 1; // +1 for 'S'
            curr_pos_idx = b_idx;
            ...
            rep(d, 4) dp[d] = next_dp_bk[d] + 1; // +1 for 'S'
            curr_pos_idx = bk_idx;
        }
    ...
    vector<pair<int, vector<int>>> history; // pos_idx, dp_values
